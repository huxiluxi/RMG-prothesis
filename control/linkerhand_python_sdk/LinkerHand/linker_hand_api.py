...
from control.linkerhand_python_sdk.LinkerHand.utils.mapping import *
from control.linkerhand_python_sdk.LinkerHand.utils.color_msg import ColorMsg
from control.linkerhand_python_sdk.LinkerHand.utils.load_write_yaml import LoadWriteYaml
from control.linkerhand_python_sdk.LinkerHand.utils.open_can import OpenCan

class LinkerHandApi:
    def __init__(self, hand_type="right", hand_joint="L7", modbus = "None",can="can0"):  # Ubuntu:can0   win:PCAN_USBBUS1
        self.last_position = []
        ...
        if self.hand_joint.upper() == "O6":
            if modbus != "None":
                from control.linkerhand_python_sdk.LinkerHand.core.rs485.linker_hand_o6_rs485 import LinkerHandO6RS485
                self.hand = LinkerHandO6RS485(hand_id=self.hand_id,modbus_port=modbus,baudrate=115200)
            else:
                from control.linkerhand_python_sdk.LinkerHand.core.can.linker_hand_o6_can import LinkerHandO6Can
                self.hand = LinkerHandO6Can(can_id=self.hand_id,can_channel=self.can, yaml=self.yaml)
        if self.hand_joint == "L6":
            if modbus != "None":
                from control.linkerhand_python_sdk.LinkerHand.core.rs485.linker_hand_l6_rs485 import LinkerHandL6RS485
                self.hand = LinkerHandL6RS485(hand_id=self.hand_id,modbus_port=modbus,baudrate=115200)
            else:
                from control.linkerhand_python_sdk.LinkerHand.core.can.linker_hand_l6_can import LinkerHandL6Can
                self.hand = LinkerHandL6Can(can_id=self.hand_id,can_channel=self.can, yaml=self.yaml)
        if self.hand_joint == "L7":
            if modbus != "None":
                from control.linkerhand_python_sdk.LinkerHand.core.rs485.linker_hand_l7_rs485 import LinkerHandL7RS485
                self.hand = LinkerHandL7RS485(hand_id=self.hand_id,modbus_port=modbus,baudrate=115200)
            else:
                from control.linkerhand_python_sdk.LinkerHand.core.can.linker_hand_l7_can import LinkerHandL7Can
                self.hand = LinkerHandL7Can(can_id=self.hand_id,can_channel=self.can, yaml=self.yaml)
        if self.hand_joint == "L10":
            if modbus != "None":
                from control.linkerhand_python_sdk.LinkerHand.core.rs485.linker_hand_l10_rs485 import LinkerHandL10RS485
                self.hand = LinkerHandL10RS485(hand_id=self.hand_id,modbus_port=modbus,baudrate=115200)
            else:
                from control.linkerhand_python_sdk.LinkerHand.core.can.linker_hand_l10_can import LinkerHandL10Can
                self.hand = LinkerHandL10Can(can_id=self.hand_id,can_channel=self.can, yaml=self.yaml)
        if self.hand_joint == "L20":
            from control.linkerhand_python_sdk.LinkerHand.core.can.linker_hand_l20_can import LinkerHandL20Can
            self.hand = LinkerHandL20Can(can_id=self.hand_id,can_channel=self.can, yaml=self.yaml)
        if self.hand_joint == "G20":
            from control.linkerhand_python_sdk.LinkerHand.core.can.linker_hand_g20_can import LinkerHandG20Can
            self.hand = LinkerHandG20Can(can_id=self.hand_id,can_channel=self.can, yaml=self.yaml)
        if self.hand_joint == "L21":
            from control.linkerhand_python_sdk.LinkerHand.core.can.linker_hand_l21_can import LinkerHandL21Can
            self.hand = LinkerHandL21Can(can_id=self.hand_id,can_channel=self.can, yaml=self.yaml)
        if self.hand_joint == "L25":
            from control.linkerhand_python_sdk.LinkerHand.core.can.linker_hand_l25_can import LinkerHandL25Can
